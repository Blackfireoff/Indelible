...
export async function runPipeline(
  ...
  // -------------------------------------------------------------------------
  // Step 4: Embed query and retrieve top-K chunks
  // -------------------------------------------------------------------------
  ...
  const scoredChunks = allChunks
    .map((chunk) => {
      // In production, use precomputed embeddings from 0G
      // For now, we assign a simple score based on speaker/topic match
      let score = 0.5; // Base score

      // Boost chunks that match the speaker
      if (intent.parsed.speaker && chunk.speaker) {
        const querySpeaker = intent.parsed.speaker.toLowerCase();
        const chunkSpeaker = chunk.speaker.toLowerCase();
        if (chunkSpeaker.includes(querySpeaker) || querySpeaker.includes(chunkSpeaker)) {
          score += 0.3;
        }
      }

      // Boost chunks that contain topic keywords
      if (intent.parsed.topic) {
        const topicWords = intent.parsed.topic.toLowerCase().split(/\s+/);
        const chunkText = chunk.text.toLowerCase();
        const topicMatches = topicWords.filter((w) => chunkText.includes(w)).length;
