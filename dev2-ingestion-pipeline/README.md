...
| `ZEROG_UPLOAD_ARTIFACT_ATTEMPTS` | `3` | Après les retries **internes** au SDK (`ZEROG_UPLOAD_MAX_RETRIES`), nombre de tentatives complètes par fichier (nouveau Merkle + tx) en cas d’erreur ; mettre `1` pour désactiver |
| `ZEROG_UPLOAD_ARTIFACT_RETRY_BASE_MS` | `4000` | Délai de base (ms) entre tentatives externes ; backoff exponentiel (`base × 2^(attempt-1)`) |
...
- Small files are padded to ≥ 2 KB (storage node preference); optional env: `ZEROG_UPLOAD_MAX_RETRIES`, `ZEROG_UPLOAD_ARTIFACT_ATTEMPTS` (outer retries after reverts), `ZEROG_GAS_PRICE`, `ZEROG_GAS_LIMIT`, `ZEROG_INDEXER_SYNC_TIMEOUT_MS`, `ZEROG_INDEXER_SYNC_INTERVAL_MS` (see `.env.example`)
